...
class ZSetScoredBytesMember {
  final Uint8List member;
  final double score;

  ZSetScoredBytesMember(this.member, this.score);
}
...
class ZSetPopBytesResult {
  final String key;
  final List<ZSetScoredBytesMember> entries;

  ZSetPopBytesResult(this.key, this.entries);
}
...
mixin RedisSortedSetCommands on RedisCommandExecutor {
  ...
  Future<List<Uint8List>> zRandMemberBytes(String key, [int? count]) async {
    final args = <dynamic>['ZRANDMEMBER', key];
    if (count != null) args.add(count);
    final res = await sendCommand(args);
  ...
  Future<List<Uint8List>> zRangeBytes(
    String key,
    int start,
    int stop, {
    bool withScores = false,
  ...
  Future<List<Uint8List>> zRevRangeBytes(
    String key,
    int start,
    int stop, {
    bool withScores = false,
  ...
  Future<List<Uint8List>> zRangeByScoreBytes(
    String key,
    dynamic min,
    dynamic max, {
    bool withScores = false,
  ...
  Future<List<Uint8List>> zRevRangeByScoreBytes(
    String key,
    dynamic max,
    dynamic min, {
    bool withScores = false,
  ...
  Future<List<Uint8List>> zRangeByLexBytes(
    String key,
    String min,
    String max, {
    int? offset,
  ...
  Future<List<Uint8List>> zRevRangeByLexBytes(
    String key,
    String max,
    String min, {
    int? offset,
  ...
  Future<List<Uint8List>> zInterBytes(
    int numKeys,
    List<String> keys, {
    bool withScores = false,
  }) async {
  ...
  Future<List<Uint8List>> zUnionBytes(
    int numKeys,
    List<String> keys, {
    bool withScores = false,
  }) async {
  ...
  Future<List<Uint8List>> zDiffBytes(int numKeys, List<String> keys) async {
    final res = await sendCommand(['ZDIFF', numKeys, ...keys]);
    if (res is! List) return const [];
    return res.map(Decoders.bytes).toList(growable: false);
  }
  ...
  Future<List<String>> zPopMin(String key, [int? count]) async {
    final args = <dynamic>['ZPOPMIN', key];
    if (count != null) args.add(count);
    final res = await sendCommand(args);
    if (res is List) return res.map(Decoders.string).toList();
    return [];
  ...
  Future<List<Uint8List>> zPopMinBytes(String key, [int? count]) async {
    final args = <dynamic>['ZPOPMIN', key];
    if (count != null) args.add(count);
    final res = await sendCommand(args);
    if (res is! List) return const [];
  ...
  Future<List<String>> zPopMax(String key, [int? count]) async {
    final args = <dynamic>['ZPOPMAX', key];
    if (count != null) args.add(count);
    final res = await sendCommand(args);
    if (res is List) return res.map(Decoders.string).toList();
    return [];
  ...
  Future<List<Uint8List>> zPopMaxBytes(String key, [int? count]) async {
    final args = <dynamic>['ZPOPMAX', key];
    if (count != null) args.add(count);
    final res = await sendCommand(args);
    if (res is! List) return const [];
  ...
  Future<ZSetPopBytesResult?> zMPopBytes(
    List<String> keys,
    String where, {
    int? count,
  }) async {
    ...
    if (count != null) args.addAll(['COUNT', count]);
    final res = await sendCommand(args);
    return _parseZSetPopBytesResult(res);
  }
  ...
  Future<ZSetPopBytesResult?> bZMPopBytes(
    int timeout,
    List<String> keys,
    String where, {
    int? count,
  ...
  Future<ZSetPopBytesResult?> bZPopMinBytes(List<String> keys, int timeout) async {
    final res = await sendCommand(['BZPOPMIN', ...keys, timeout]);
    return _parseBlockingZSetPopBytesResult(res);
  }
  ...
  Future<ZSetPopBytesResult?> bZPopMaxBytes(List<String> keys, int timeout) async {
    final res = await sendCommand(['BZPOPMAX', ...keys, timeout]);
    return _parseBlockingZSetPopBytesResult(res);
  }
  ...
  Future<ScanResult<MapEntry<Uint8List, double>>> zScanBytes(
    String key,
    int cursor, {
    String? match,
    int? count,
    ...
    if (res is List && res.length == 2 && res[1] is List) {
      final nextCursor = Decoders.toInt(res[0]);
      final list = res[1] as List;
      final entries = <MapEntry<Uint8List, double>>[];
      for (var i = 0; i < list.length; i += 2) {
        final member = Decoders.bytes(list[i]);
        final score = Decoders.toDouble(list[i + 1]);
        entries.add(MapEntry(member, score));
      }
      return ScanResult(nextCursor, entries);
  ...
  ZSetPopBytesResult? _parseZSetPopBytesResult(dynamic res) {
    if (res == null) return null;
    if (res is List && res.length == 2 && res[1] is List) {
      return ZSetPopBytesResult(
        Decoders.string(res[0]),
        _parseScoredBytesMembers(res[1]),
      );
    }
    throw DaredisProtocolException('Unexpected sorted-set pop response: $res');
  }
  ...
  ZSetPopBytesResult? _parseBlockingZSetPopBytesResult(dynamic res) {
    if (res == null) return null;
    if (res is List && res.length == 3) {
      return ZSetPopBytesResult(
        Decoders.string(res[0]),
        [ZSetScoredBytesMember(Decoders.bytes(res[1]), Decoders.toDouble(res[2]))],
      );
    }
    throw DaredisProtocolException(
      'Unexpected blocking sorted-set pop response: $res',
  ...
  List<ZSetScoredBytesMember> _parseScoredBytesMembers(dynamic value) {
    if (value is! List) {
      throw DaredisProtocolException(
        'Unexpected sorted-set member list response: $value',
      );
    ...
    return value.map((entry) {
      if (entry is List && entry.length == 2) {
        return ZSetScoredBytesMember(
          Decoders.bytes(entry[0]),
          Decoders.toDouble(entry[1]),
        );
      }
      throw DaredisProtocolException(
